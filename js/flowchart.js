...
!function(root, factory) {
  ...
  \*********************************/
    ...
    function(module, exports, __webpack_require__) {
        ...
        Symbol.prototype.getAttr = function(attName) {
            ...
            var line, x = this.getCenter().x, y = this.getCenter().y, top = this.getTop(), right = this.getRight(), bottom = this.getBottom(), left = this.getLeft(), symbolX = symbol.getCenter().x, symbolY = symbol.getCenter().y, symbolTop = symbol.getTop(), symbolBottom = symbol.getBottom(), symbolRight = symbol.getRight(), symbolLeft = symbol.getLeft(), isOnSameColumn = x === symbolX, isOnSameLine = bottom.y > symbolTop.y && top.y < symbolBottom.y, // y === symbolY,
            isUnder = bottom.y < symbolTop.y, // y < symbolY && symbolTop.y < bottom,
            isUpper = y > symbolY || this === symbol, isLeft = x > symbolX, isRight = x < symbolX, maxX = 0, lineLength = this.getLineLength();
            if (// this.getAttr('line-length'),
            lineWith = this.getAttr("line-width"), origin && "bottom" !== origin || !isOnSameColumn || !isUnder) if (origin && "right" !== origin || !isOnSameLine || !isRight) if (origin && "left" !== origin || !isOnSameLine || !isLeft) if (origin && "right" !== origin || !isOnSameColumn || !isUpper) if (origin && "right" !== origin || !isOnSameColumn || !isUnder) if (origin && "bottom" !== origin || !isLeft) if (origin && "bottom" !== origin || !isRight) if (origin && "right" === origin && isLeft) line = drawLine(this.chart, right, [ {
  ...
  \********************************/
    ...
    function(module, exports, __webpack_require__) {
        ...
        var FlowChart = __webpack_require__(/*! ./flowchart.chart */ 6), Start = __webpack_require__(/*! ./flowchart.symbol.start */ 14), End = __webpack_require__(/*! ./flowchart.symbol.end */ 11), Operation = __webpack_require__(/*! ./flowchart.symbol.operation */ 13), Database = __webpack_require__(/*! ./flowchart.symbol.database */ 10), InputOutput = __webpack_require__(/*! ./flowchart.symbol.inputoutput */ 12), Subroutine = __webpack_require__(/*! ./flowchart.symbol.subroutine */ 15), Condition = __webpack_require__(/*! ./flowchart.symbol.condition */ 5), Cloud = __webpack_require__(/*! ./flowchart.symbol.cloud */ 9);
  ...
  \********************************/
    ...
    function(module, exports, __webpack_require__) {
        ...
        var Raphael = __webpack_require__(/*! raphael */ 17), defaults = __webpack_require__(/*! ./flowchart.helpers */ 1).defaults, defaultOptions = __webpack_require__(/*! ./flowchart.defaults */ 7), Condition = __webpack_require__(/*! ./flowchart.symbol.condition */ 5);
  ...
  \***************************************/
    ...
    function(module, exports, __webpack_require__) {
        function Cloud(chart, options) {
            options = options || {}, Symbol.call(this, chart, options);
            var originX = 0, originY = 0;
            this.textMargin = this.getAttr("text-margin");
            // Set width to box width increased by 4 times the text margin
            var textWidth = this.text.getBBox().width + 2 * this.textMargin, textHeight = this.text.getBBox().height + 2 * this.textMargin, b = .75 * textWidth, h = .25 * textWidth, ngl = Math.cos(.3762), coeff = 1.0752, r = coeff * b / 2 / ngl, topBubbleHeight = h / 2 + r * (1 - Math.sin(.3762)), r3 = .9 * h, cv1r = textHeight / 1.95, cv1y1 = originY + topBubbleHeight + textHeight, cv1x = originX + textHeight / 2.5, cv1y2 = originY + topBubbleHeight;
            cv1 = "M" + cv1x + "," + cv1y1 + " A" + cv1r + "," + cv1r + " 0 0,1 " + cv1x + "," + cv1y2, 
            cv2 = "A" + r + "," + r + " 0 0,1 " + (b + cv1x) + "," + (cv1y2 - h), cv3 = "A" + r3 + "," + r3 + " 0 0,1 " + (textWidth + cv1x) + "," + cv1y2, 
            cv4 = "A" + cv1r + "," + cv1r + " 0 0,1 " + (textWidth + cv1x) + "," + cv1y1, base = "H " + cv1x;
            ...
            }), this.startRight = textWidth + cv1x, this.initialize();
        }
        var Symbol = __webpack_require__(/*! ./flowchart.symbol */ 2), inherits = __webpack_require__(/*! ./flowchart.helpers */ 1).inherits, drawAPI = __webpack_require__(/*! ./flowchart.functions */ 3);
        drawAPI.drawPath;
        inherits(Cloud, Symbol), Cloud.prototype.getLeft = function() {
            var x = this.getX() + this.textMargin, y = this.getY() + this.group.getBBox().height / 2;
            return {
                x: x,
                y: y
            };
        }, Cloud.prototype.getRight = function() {
            var x = this.getX() + this.startRight, y = this.getY() + this.group.getBBox().height / 2;
            return {
                x: x,
                y: y
            };
  ...
  \******************************************/
    ...
    function(module, exports, __webpack_require__) {
        function Database(chart, options) {
            options = options || {}, Symbol.call(this, chart, options), this.textMargin = this.getAttr("text-margin");
            // Set width to box width increased by 4 times the text margin
            var width = this.text.getBBox().width + 2 * this.textMargin, ellipseHeight = width / 6.5, height = this.text.getBBox().height + 2 * this.textMargin + ellipseHeight / 2, bottomEllipse = chart.paper.ellipse(width / 2, height + ellipseHeight, width / 2, ellipseHeight / 2);
            bottomEllipse.attr({
                stroke: this.getAttr("element-color"),
                "stroke-width": this.getAttr("line-width"),
                fill: this.getAttr("fill")
            ...
            var rightBorder = drawPath(chart, {
                x: width,
                y: ellipseHeight / 2
            }, [ {
                x: width,
            ...
            rightBorder.attr({
                stroke: this.getAttr("element-color"),
                "stroke-width": this.getAttr("line-width")
            }), console.debug(this.group.getBBox()), this.group.push(leftBorder, bottomEllipse, textArea, topEllipse, rightBorder), 
            //(symbol, topEllipse);
            console.debug(this.group.getBBox()), bottomEllipse.insertBefore(this.text), textArea.insertBefore(this.text), 
            topEllipse.insertBefore(this.text), this.text.attr({
                y: this.group.getBBox().height / 2
            }), this.initialize();
        }
        var Symbol = __webpack_require__(/*! ./flowchart.symbol */ 2), inherits = __webpack_require__(/*! ./flowchart.helpers */ 1).inherits, drawAPI = __webpack_require__(/*! ./flowchart.functions */ 3), drawPath = drawAPI.drawPath;
        inherits(Database, Symbol), Database.prototype.getLeft = function() {
            var x = this.getX() + this.textMargin, y = this.getY() + this.group.getBBox().height / 2;
            return {
                x: x,
                y: y
            ...
            var x = this.getX() + this.group.getBBox().width, y = this.getY() + this.group.getBBox().height / 2;
            return {
                x: x,
                y: y
            };
